...
function TagGroup({ icon, items, colorClass }: TagGroupProps) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm md:text-base font-medium mt-0.5">{items.join("  ")}</span>
    </div>
  )
}
...
export default function SongHeader({ song, onShare }: SongHeaderProps) {
  ...
  const allTags = [
    { icon: <Globe className="w-4 h-4 md:w-5 md:h-5" />, items: translatedCountries, colorClass: "bg-blue-50 text-blue-700" },
    { icon: <MessageCircle className="w-4 h-4 md:w-5 md:h-5" />, items: translatedLanguages, colorClass: "bg-green-50 text-green-700" },
    { icon: <Music className="w-4 h-4 md:w-5 md:h-5" />, items: translatedGenres, colorClass: "bg-purple-50 text-purple-700" },
    { icon: <Users className="w-4 h-4 md:w-5 md:h-5" />, items: translatedAudience, colorClass: "bg-orange-50 text-orange-700" },
    { icon: <Mountain className="w-4 h-4 md:w-5 md:h-5" />, items: [translatedDifficulty], colorClass: "bg-amber-50 text-amber-700" },
    { icon: <Sparkles className="w-4 h-4 md:w-5 md:h-5" />, items: translatedThemes, colorClass: "bg-rose-50 text-rose-700" },
      ...
      <div className="flex items-start gap-4 md:gap-8">
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="flex items-start justify-between gap-2 md:gap-4 mb-2 md:mb-5">
            <h1 className="text-lg md:text-4xl font-bold text-[#18375E] leading-tight break-words">{song.title}</h1>
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 md:pr-8 overflow-hidden">
            ...
            {!showAllTags && allTags.slice(3).map((tag, index) => (
